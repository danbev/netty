...
public final class SockJsAsserts {
    ...
    private static final int CONTENT_SIZE = 2048;
    ...
    public static void assertDefaultResponseHeaders(final HttpResponse response, final String contentType) {
        ...
        assertThat(headers.get(CONTENT_TYPE), equalTo(contentType));
    ...
    public static void assertNoCache(final HttpResponse response) {
        final HttpHeaders headers = response.headers();
        assertThat(headers.get(CACHE_CONTROL), containsString("no-store"));
        assertThat(headers.get(CACHE_CONTROL), containsString("no-cache"));
        assertThat(headers.get(CACHE_CONTROL), containsString("must-revalidate"));
        assertThat(headers.get(CACHE_CONTROL), containsString("max-age=0"));
    }

    public static void assertOrigin(final HttpResponse response, final String origin) {
        final HttpHeaders headers = response.headers();
        assertThat(headers.get(ACCESS_CONTROL_ALLOW_ORIGIN), equalTo(origin));
    }
    ...
    public static void assertAccessRequestAllowHeaders(final HttpResponse response, final String headers) {
        assertThat(response.headers().get(ACCESS_CONTROL_ALLOW_HEADERS), equalTo(headers));
    }
    ...
    public static void assertAccessControlAllowCredentials(final HttpResponse response, final boolean value) {
        final HttpHeaders headers = response.headers();
        if (value) {
            assertThat(headers.get(ACCESS_CONTROL_ALLOW_CREDENTIALS), is("true"));
        } else {
            assertThat(headers.get(ACCESS_CONTROL_ALLOW_CREDENTIALS), is(nullValue()));
        }
    }
    ...
    public static void assertWebSocketUpgradeResponse(final EmbeddedChannel ch) throws Exception {
        ...
        final String connection = upgradeResponse.headers().get(CONNECTION);
        assertThat(connection.equalsIgnoreCase(Values.UPGRADE), is(true));
    ...
    public static void assertEntityTag(final FullHttpResponse response, final String expected) {
        assertThat(response.headers().get(ETAG), equalTo(expected));
        release(response);
    }

    public static void assertPrelude(final HttpContent httpContent) {
        assertPrelude(httpContent.content(), CONTENT_SIZE, true);
    }

    public static void assertPrelude(final PreludeFrame preludeFrame) {
        assertPrelude(preludeFrame.content(), CONTENT_SIZE, false);
    }

    public static void assertPrelude(final ByteBuf buf, final int size, final boolean newLine) {
        final int totalSize = newLine ? size + 1 : size;
        assertThat(buf.readableBytes(), is(totalSize));
        assertThat(buf.toString(UTF_8), equalTo(generatePrelude(totalSize, newLine)));
        release(buf);
    }
    ...
    private static String generatePrelude(final int size, final boolean newLine) {
        final int chars = newLine ? size - 2 : size - 1;
