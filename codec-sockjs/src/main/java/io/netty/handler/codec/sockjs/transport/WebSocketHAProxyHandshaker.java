...
import static io.netty.handler.codec.http.HttpHeaderNames.SEC_WEBSOCKET_KEY1;
import static io.netty.handler.codec.http.HttpHeaderNames.SEC_WEBSOCKET_KEY2;
import static io.netty.handler.codec.http.HttpHeaderNames.SEC_WEBSOCKET_ORIGIN;
import static io.netty.handler.codec.http.HttpHeaderNames.SEC_WEBSOCKET_LOCATION;
...
import static io.netty.handler.codec.http.HttpHeaderNames.WEBSOCKET_ORIGIN;
import static io.netty.handler.codec.http.HttpHeaderNames.WEBSOCKET_PROTOCOL;
import static io.netty.handler.codec.http.HttpHeaderNames.WEBSOCKET_LOCATION;
...
public class WebSocketHAProxyHandshaker extends WebSocketServerHandshaker00 {
    ...
    private static final FastThreadLocal<MessageDigest> MESSAGE_DIGEST = new FastThreadLocal<MessageDigest>() {
        @Override
    ...
    protected FullHttpResponse newHandshakeResponse(FullHttpRequest req, HttpHeaders headers) {
        ...
        if (!AsciiString.equalsIgnoreCase(UPGRADE, req.headers().get(CONNECTION))
                || !AsciiString.equalsIgnoreCase(WEBSOCKET, req.headers().get(UPGRADE))) {
        ...
        res.headers().add(CONNECTION, HttpHeaderValues.UPGRADE);
     ...
     */
    public static boolean isHAProxyRequest(final FullHttpRequest request) {
        final String version = request.headers().get(SEC_WEBSOCKET_VERSION);
