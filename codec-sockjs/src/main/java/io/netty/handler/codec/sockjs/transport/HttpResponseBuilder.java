...
public final class HttpResponseBuilder {
    ...
    /**
     * Sets the HTTP header {@link HttpHeaderNames#SET_COOKIE} if {@link SockJsConfig#areCookiesNeeded()}
     * is true.
     *
    ...
    public HttpResponseBuilder setCookie(final SockJsConfig config) {
        if (config.areCookiesNeeded()) {
            header(HttpHeaderNames.SET_COOKIE, encodeSessionIdCookie(request));
        }
        return this;
    }
    ...
    private static String encodeSessionIdCookie(final HttpRequest request) {
        ...
        if (cookieHeader != null) {
            final Set<Cookie> cookies = ServerCookieDecoder.decode(cookieHeader);
