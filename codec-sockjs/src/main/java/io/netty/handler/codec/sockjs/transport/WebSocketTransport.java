...
public class WebSocketTransport extends ChannelInboundHandlerAdapter {
    ...
    private static boolean checkRequestHeaders(final ChannelHandlerContext ctx, final HttpRequest req) {
        ...
        final String upgradeHeader = req.headers().get(HttpHeaderNames.UPGRADE);
    ...
    private void handleHttpRequest(final ChannelHandlerContext ctx, FullHttpRequest req) {
        try {
            ...
            if (WebSocketHAProxyHandshaker.isHAProxyRequest(req)) {
                final String wsUrl = webSocketLocation(config.isTls(), req);
    ...
    private static String webSocketLocation(final boolean tls, final FullHttpRequest req) {
